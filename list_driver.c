...
int list_add_entry(const char *name, int pid, int n_msg){
	...
	if (!new_node) {
		printk(KERN_INFO "MQ_DRIVER: Memory allocation failed, this should never fail due to GFP_KERNEL flag\n");
		return -1;
	}
...
int list_add_msg_entry(const char *name, const char *data, int size){
	...
	struct message_s *entry = NULL, *tmp = NULL;
	int count_m = 0;
	...
	if (data == NULL || strlen(data) == 0) {
   		printk(KERN_ERR "MQ_DRIVER: Dados da mensagem são NULL, abortando.\n");
    	return 1;
	}
	...
	if (!new_node->message) {
		printk(KERN_INFO "MQ_DRIVER: Memory allocation failed for message string\n");
		return -1;
	}
	...
	if(target->count_msg == target->n_msg){ //Caso a lista esta cheia, substitui head
		INIT_LIST_HEAD(&new_node->link);
		list_delete_head_msg(target);
		list_add(&new_node->link, &target->list_m);
		target->count_msg = 1;
	}else{ //Caso lista cheia substitui nodo
		target->count_msg += 1;
		list_for_each_entry_safe(entry, tmp, &target->list_m, link) {
			if(entry->message == NULL) break;
			count_m++;
			if(count_m == target->count_msg){
				list_replace(&entry->link, &new_node->link);
				kfree(entry->message); 
				kfree(entry);
				return 0;
			}
...
int list_delete_head_msg(struct process * process){
	...
	if (list_empty(&process->list_m)) {
		printk(KERN_INFO "MQ_DRIVER: Empty list.\n");
...
int list_delete_entry(struct process * process){
	...
	list_for_each_entry_safe(entry, tmp, &process->list_m, link) {
		if (entry->message)
			printk(KERN_INFO "MQ_DRIVER: Remove message: %s", entry->message);
		else
			printk(KERN_INFO "MQ_DRIVER: Remove message: <NULL>");
	...
	printk(KERN_INFO "MQ_DRIVER: Remove PROCESS: %s", process->name);
